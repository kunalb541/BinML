...
# v4.2.0: Process-local VBBinaryLensing singleton. Under multiprocessing each worker
# process lazily builds its own instance on first use, so there is no cross-process
# sharing and the configured tolerance is applied exactly once per worker.
_VBB_INSTANCE = None


def _get_vbb() -> "VBBinaryLensing.VBBinaryLensing":
    """Return this process's cached VBBinaryLensing instance (built on first call)."""
    global _VBB_INSTANCE
    if _VBB_INSTANCE is None:
        _VBB_INSTANCE = VBBinaryLensing.VBBinaryLensing()
        _VBB_INSTANCE.Tol = SimConfig.VBM_TOLERANCE
    return _VBB_INSTANCE
...
def binary_magnification_vbb(
    ...
    """
    # v4.2.0: Reuse a process-local VBBinaryLensing instance. Construction is cheap
    # (~2e-7 s) but re-creating it per event is pure churn at 10^7-10^8 event scale.
    VBB = _get_vbb()
    ...
    # v4.2.0: BinaryMag2 is the per-point finite-source solver (adaptive: it self-
    # switches to full contour integration only near caustics). Point-by-point is
    # required here because VBBinaryLensing's array/light-curve APIs use a different
    # trajectory convention that does NOT reproduce this (u1, u2) parameterization
    # (verified: up to ~16x per-point disagreement), and it is only ~1.2x faster.
    # The previously-attempted VBB.BinaryMag(array) "fast path" never accepted arrays
    # and raised on every event before falling through to exactly this loop.
    n = len(t)
    mag = np.ones(n, dtype=np.float32)
    bin_mag2 = VBB.BinaryMag2
    for i in range(n):
        try:
            val = bin_mag2(s, q, float(u1[i]), float(u2[i]), rho)
            if val > 0 and np.isfinite(val):
                mag[i] = val
        except (RuntimeError, ValueError, TypeError):
            # Use PSPL approximation for this point (do NOT catch MemoryError)
            u_sq = u1[i]**2 + u2[i]**2
            u = np.sqrt(u_sq)
...
def unpack_mask(packed: np.ndarray, n_points: int = None) -> np.ndarray:
    """
    Reconstruct the boolean cadence mask from the packed uint8 bitmask.

    v4.2.0: The HDF5 "mask" dataset stores each event's per-point valid-observation
    mask as packed bits (np.packbits along the time axis). This reverses that. Use
    together with a global time grid to regenerate delta_t exactly:

        time_grid = generate_time_grid()
        mask = unpack_mask(f['mask'][i], f.attrs['n_points'])
        delta_t = compute_delta_t(time_grid, mask)

    Parameters
    ----------
    packed : np.ndarray
        uint8 array, shape (..., ceil(n_points/8)), from the "mask" dataset.
    n_points : int, optional
        Number of time points to trim to (removes packbits tail padding). Defaults
        to SimConfig.N_POINTS. 6912 is a multiple of 8, so no padding exists then.

    Returns
    -------
    np.ndarray
        Boolean mask, shape (..., n_points). True = valid observation.
    """
    if n_points is None:
        n_points = SimConfig.N_POINTS
    bits = np.unpackbits(np.asarray(packed, dtype=np.uint8), axis=-1)
    return bits[..., :n_points].astype(bool)
...
def simulate_event(params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if etype == 'flat':
        ...
        p = dict(BinaryPresets.PRESETS[params['preset']])  # copy so overrides don't mutate global
        preset_name = params['preset']
        # v4.2.0: per-regime fine-tuning overrides threaded via params
        q_ov = params.get('q_override')
        if q_ov is not None:
            lo, hi = q_ov
            base_lo, base_hi = p['q_range']
            p['q_range'] = (lo if lo else base_lo, hi if hi else base_hi)
    ...
    # v4.2.0: Per-event detectability metrics for hard-example selection.
    # These are intrinsic (computed on the noiseless curve A vs the per-point noise
    # level, before cadence masking) so they are reproducible and mask-independent.
    #   - snr: peak signal-to-noise of the lensing deviation from baseline.
    #   - anomaly_dchi2 / max_anomaly (binary only): how far the true binary curve
    #     departs from the matched single-lens (same u0, tE, t0). This is THE
    #     discriminator between "detectable" caustic binaries and "smooth" PSPL-like
    #     binaries, and is what the selection pipeline should oversample on.
    sigma_A = np.maximum(noise_jy / f_base, 1e-12)  # per-point noise in mag units
    # peak magnification and lensing SNR are defined for every event type.
    meta['peak_magnification'] = float(np.max(A))
    meta['snr'] = float(np.max(np.abs(A - 1.0) / sigma_A))
    # anomaly_dchi2 / max_anomaly are binary-only (deviation from the matched single
    # lens); leaving them absent for flat/PSPL stores them as NaN in the aligned
    # params table, consistent with q/s/rho and keeping per-regime bins binary-pure.
    if etype == 'binary':
        A_pspl_matched = pspl_magnification(t_grid, tE, u0, t0)
        A_pspl_matched = np.minimum(A_pspl_matched, PSPL_MAX_MAGNIFICATION)
        resid = A - A_pspl_matched
        meta['anomaly_dchi2'] = float(np.sum((resid / sigma_A) ** 2))
        meta['max_anomaly'] = float(np.max(np.abs(resid)))
    ...
    return {
        ...
        'mask': mask.astype(bool),  # v4.2.0: valid-observation mask (True = observed)
        'label': label,
        'params': meta
    }
...
def main() -> None:
    ...
    parser.add_argument('--q_min', type=float, default=None,
                        help="Override preset q_range lower bound (for targeting a q regime)")
    parser.add_argument('--q_max', type=float, default=None,
                        help="Override preset q_range upper bound (for targeting a q regime)")
    parser.add_argument('--force_caustic', action='store_true',
                        help="Override the preset's require_caustic flag to True (favor detectable bumps)")
    ...
    base_params: Dict[str, Any] = {
        ...
        # v4.2.0: per-regime fine-tuning overrides (None => use preset defaults)
        'q_override': (args.q_min, args.q_max) if (args.q_min or args.q_max) else None,
        'force_caustic': bool(args.force_caustic),
    }
    ...
    # v4.2.0: two dense f4 arrays (flux, delta_t) + packed mask (~n_points/8 bytes);
    # time_grid stored once, per-event timestamps dropped.
    est_size_gb = (n_res * n_points * 4 * 2 + n_res * (n_points // 8)) / 1e9
    ...
    # v4.2.0: Pre-scan the union of numeric parameter fields across ALL events so a
    # single global, row-aligned `params` table can be written in shuffled order.
    # This is the provenance fix: per-class `params_{class}` arrays are NOT indexable
    # by global row, but this table is. Fields absent for a class (e.g. q, s for a
    # flat/PSPL event) are stored as NaN so they are distinguishable from real zeros.
    global_param_fields: set = set()
    for r in final_results:
        for k, v in r['params'].items():
            if isinstance(v, (int, float, np.number)):
                global_param_fields.add(k)
    global_param_fields = sorted(global_param_fields)
    global_param_dtype = np.dtype([(fld, 'f8') for fld in global_param_fields])

    # v4.2.0: Packed cadence bitmask. n_points is a multiple of 8 (6912 = 864*8),
    # so np.packbits/unpackbits round-trip with no padding. Reconstruct delta_t in
    # the loader via compute_delta_t(time_grid, unpacked_mask).
    n_mask_bytes = (n_points + 7) // 8
    ...
    with h5py.File(out_path, 'w') as f:
        ...
        # v4.2.0: Store the observation time grid ONCE instead of an identical
        # per-event `timestamps` row (n_res x 6912). Removes ~1/3 of dense storage.
        # Kept float64 (only 6912 values, stored once) so delta_t reconstructed via
        # compute_delta_t(time_grid, mask) is bit-exact against the stored delta_t.
        f.create_dataset('time_grid', data=np.asarray(time_grid, dtype=np.float64))
        ds_mask = f.create_dataset(
            'mask', shape=(n_res, n_mask_bytes), dtype='u1',
            compression='lzf', chunks=(min(SAVE_CHUNK_SIZE, n_res), n_mask_bytes)
        )
        ds_params = f.create_dataset(
            'params', shape=(n_res,), dtype=global_param_dtype, compression='lzf',
            chunks=(min(SAVE_CHUNK_SIZE, n_res),)
        )
        # Record the global param field order for loaders (structured-dtype names are
        # preserved by h5py, but this attr documents them explicitly).
        f.attrs['param_fields'] = list(global_param_fields)
        ...
        for start_idx in tqdm(range(0, n_res, SAVE_CHUNK_SIZE), 
            ...
            mask_chunk = np.zeros((chunk_size, n_points), dtype=bool)
            # Row-aligned global params (NaN = field not applicable to this event's class)
            params_chunk = np.zeros(chunk_size, dtype=global_param_dtype)
            for fld in global_param_fields:
                params_chunk[fld] = np.nan
            ...
            for i, r in enumerate(final_results[start_idx:end_idx]):
                ...
                mask_chunk[i] = r['mask']
                for fld, val in r['params'].items():
                    if fld in global_param_dtype.names:
                        params_chunk[i][fld] = val
            ...
            # Pack the boolean mask along the time axis (6912 bits -> 864 bytes/event)
            ds_mask[start_idx:end_idx] = np.packbits(mask_chunk, axis=1)
            ds_params[start_idx:end_idx] = params_chunk
        
        # v4.2.0: per-event `timestamps` dropped; single `time_grid` written above.
        ...
        metadata = {
            ...
            'version': __version__,
            'mask_packed': True,
            'mask_n_bytes': int(n_mask_bytes),
            'has_global_params': True,
            'has_time_grid': True,
            'timestamps_dropped': True,
            'note': ('v4.2.0: Packed cadence mask ("mask") + global row-aligned params '
                     '("params") + detectability metrics; per-event "timestamps" dropped, '
                     'single "time_grid" stored; reconstruct times by tiling time_grid and '
                     'delta_t via compute_delta_t(time_grid, unpacked_mask)')
        }
