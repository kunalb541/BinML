...
__version__: Final[str] = "4.2.0"  # v4.2.0: numeric ECE/MCE/Brier calibration metrics + calibration_metrics.json
    ...
    if suffix == '.h5':
        ...
        with h5py.File(data_path, 'r') as f:
            ...
            if 'timestamps' in f:
                ...
                # v4.2.0 compact format: single stored grid, tiled to per-event rows.
                n_samples_total, seq_len = flux.shape
                timestamps = np.tile(f['time_grid'][:].astype(np.float32),
                                     (n_samples_total, 1))
            else:
                n_samples_total, seq_len = flux.shape
                timestamps = np.tile(
...
def compute_calibration_metrics(
    y_true: np.ndarray,
    y_probs: np.ndarray,
    n_bins: int = CALIBRATION_DEFAULT_BINS,
) -> Dict[str, Any]:
    """
    Compute numeric calibration metrics from predicted probabilities.

    Previously the evaluator only *plotted* a reliability diagram; this quantifies it
    with scalars that can be tracked across runs and dropped into
    ``calibration_metrics.json``:

    - ECE (Expected Calibration Error), equal-width bins over [0, 1].
    - ECE (adaptive), equal-mass / quantile bins — more honest when confidences
      cluster near 1.0, which is exactly the overconfident regime we care about.
    - MCE (Maximum Calibration Error) for both binning schemes.
    ...
    y_probs : np.ndarray
        Predicted class probabilities, shape (n, n_classes); rows sum to ~1.
    n_bins : int
    ...
    -------
    dict
        Scalar calibration metrics plus per-bin reliability tables.
    """
    y_true = np.asarray(y_true).astype(int)
    y_probs = np.asarray(y_probs, dtype=np.float64)
    n = len(y_true)
    if n == 0:
        return {'ece': 0.0, 'ece_adaptive': 0.0, 'mce': 0.0, 'mce_adaptive': 0.0,
                'brier_multiclass': 0.0, 'n_bins': int(n_bins)}

    confidences = y_probs.max(axis=1)
    predictions = y_probs.argmax(axis=1)
    correct = (predictions == y_true).astype(np.float64)

    def _binned(bin_ids: np.ndarray, n_effective: int):
        ece, mce, reliability = 0.0, 0.0, []
        for b in range(n_effective):
            ...
            ece += (count / n) * gap
            mce = max(mce, gap)
            reliability.append({'bin': b, 'count': count, 'confidence': bin_conf,
                                'accuracy': bin_acc, 'gap': gap})
        return ece, mce, reliability

    # Equal-width bins over [0, 1]
    width_edges = np.linspace(0.0, 1.0, n_bins + 1)
    width_ids = np.clip(np.digitize(confidences, width_edges[1:-1]), 0, n_bins - 1)
    ece_w, mce_w, reliability_w = _binned(width_ids, n_bins)

    # Equal-mass (adaptive) bins via quantiles; unique() collapses ties so degenerate
    # confidence distributions do not create empty bins.
    mass_edges = np.unique(np.quantile(confidences, np.linspace(0.0, 1.0, n_bins + 1)))
    n_adaptive = max(len(mass_edges) - 1, 1)
    mass_ids = np.clip(np.digitize(confidences, mass_edges[1:-1]), 0, n_adaptive - 1)
    ece_a, mce_a, reliability_a = _binned(mass_ids, n_adaptive)

    # Multiclass Brier score
    n_classes = y_probs.shape[1]
    onehot = np.eye(n_classes)[y_true]
    brier = float(np.mean(np.sum((y_probs - onehot) ** 2, axis=1)))

    return {
        'ece': float(ece_w),
        'ece_adaptive': float(ece_a),
        'mce': float(mce_w),
        'mce_adaptive': float(mce_a),
        'brier_multiclass': brier,
        'n_bins': int(n_bins),
        'mean_confidence': float(confidences.mean()),
        'accuracy': float(correct.mean()),
        'overconfidence': float(confidences.mean() - correct.mean()),
        'reliability_equal_width': reliability_w,
        'reliability_equal_mass': reliability_a,
    }
...
def compute_comprehensive_metrics(
    ...
    # Numeric calibration metrics (ECE / MCE / Brier) — quantifies the reliability
    # diagram that was previously only plotted.
    metrics['calibration'] = compute_calibration_metrics(
        y_true, y_probs, n_bins=calibration_n_bins
    )
    ...
    if logger:
        ...
        logger.info(f" ECE: {metrics['calibration']['ece']:.4f} "
                   f"(adaptive {metrics['calibration']['ece_adaptive']:.4f}), "
                   f"MCE: {metrics['calibration']['mce']:.4f}, "
                   f"Brier: {metrics['calibration']['brier_multiclass']:.4f}")
...
class RomanEvaluator:
    ...
    def __init__(
        ...
        self.metrics = compute_comprehensive_metrics(
            ...
            seed=seed, calibration_n_bins=self.calibration_n_bins,
            logger=self.logger
        )
    ...
    def analyze_metrics_by_parameter(self) -> None:
        """
        Compute accuracy and per-class recall binned by physical/detectability
        parameters, using the v4.2.0 global row-aligned ``params`` table.

        This is the payoff of the row-aligned params fix: previously per-`q`/`s`/`u0`
        metrics required fragile within-class index remapping; now the table is
        indexable directly by global row. Satisfies the plans' requirement for
        "metrics by u0, q, s, rho, tE, peak magnification, SNR". Writes
        ``metrics_by_parameter.json``. No-op (with a log line) on older files that
        lack the aligned table.
        """
        if self.data_format != 'hdf5':
            self.logger.info("Per-parameter metrics: skipped (non-HDF5 data).")
            return

        with h5py.File(self.data_path, 'r') as f:
            if 'params' not in f or not f.attrs.get('has_global_params', False):
                self.logger.info(
                    "Per-parameter metrics: skipped (file has no aligned 'params' "
                    "table; regenerate with simulate.py >= v4.2.0)."
                )
                return
            full_params = f['params'][:]  # structured, shape (n_total,)
            field_names = list(full_params.dtype.names)

        # Align to the (possibly subsampled) evaluated rows
        sel = full_params[self.file_indices]
        y_true = np.asarray(self.y)
        y_pred = np.asarray(self.preds)
        correct = (y_true == y_pred)

        # Parameters worth slicing on, in a sensible order (only those present).
        preferred = ['u0', 'tE', 'q', 's', 'rho', 'alpha',
                     'peak_magnification', 'snr', 'anomaly_dchi2', 'max_anomaly']
        params_to_analyze = [p for p in preferred if p in field_names]

        n_bins = 8
        report: Dict[str, Any] = {'n_evaluated': int(len(y_true)), 'parameters': {}}

        for pname in params_to_analyze:
            vals = np.asarray(sel[pname], dtype=np.float64)
            valid = np.isfinite(vals)
            # anomaly/q/s/rho are binary-only -> NaN elsewhere; that is expected.
            if valid.sum() < 2 * n_bins:
                continue
            v = vals[valid]
            edges = np.unique(np.quantile(v, np.linspace(0.0, 1.0, n_bins + 1)))
            n_eff = max(len(edges) - 1, 1)
            bin_ids = np.clip(np.digitize(v, edges[1:-1]), 0, n_eff - 1)

            yt, yp, ok = y_true[valid], y_pred[valid], correct[valid]
            bins_out = []
            for b in range(n_eff):
                ...
                if count == 0:
                    continue
                entry = {
                    'lo': float(v[m].min()), 'hi': float(v[m].max()),
                    'count': count, 'accuracy': float(ok[m].mean()),
                }
                for c, cname in enumerate(CLASS_NAMES):
                    cm = m & (yt == c)
                    if cm.sum() > 0:
                        entry[f'recall_{cname}'] = float((yp[cm] == c).mean())
                bins_out.append(entry)
            report['parameters'][pname] = {
                'n_events': int(valid.sum()), 'n_bins': n_eff, 'bins': bins_out,
            }
            self.logger.info(f"  Per-parameter metrics: {pname} "
                             f"({int(valid.sum()):,} events, {n_eff} bins)")

        out_path = self.output_dir / 'metrics_by_parameter.json'
        with open(out_path, 'w') as f:
            json.dump(report, f, indent=2, cls=NumpyJSONEncoder)
        self.logger.info(f"Per-parameter metrics saved: {out_path.name} "
                         f"({len(report['parameters'])} parameters)")
    ...
    def run_all_analysis(self) -> None:
        ...
        # Per-parameter (per-regime) metrics from the aligned params table
        try:
            self.analyze_metrics_by_parameter()
        except Exception as e:
            self.logger.error(f"Failed per-parameter metrics: {e}", exc_info=True)
        ...
        # Dedicated calibration artifact (ECE / MCE / Brier + reliability tables)
        with open(self.output_dir / 'calibration_metrics.json', 'w') as f:
            json.dump(self.metrics.get('calibration', {}), f, indent=2,
