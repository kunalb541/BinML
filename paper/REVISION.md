...
| 2 | Schedule-matched augmentation: fine-tune with the exact seven fixed ~6.2 h pauses at 12.1-min cadence in a 70.7-d season (the `arm12` tree from the cadence rerun has the 12-min config) and compare to g08e12's random 1–12 h gaps | not stated | — | `validation/schedule_finetune_local.py`, `pipeline/train.py --gap-schedule rmdc26` | 🔄 running 2026-09-10: two arms on ONE pool (the cadence rerun's 15-min natural shards 0–11, 89,919 events), warm-start from the shipped weights, 12 epochs, identical except `--gap-aug 0.8` random vs `--gap-schedule rmdc26` (seven fixed pauses + the 1.3 d past the 70.7 d season end). Evaluated held-out clean, held-out under the exact schedule, and on GULLS from the cache. The 12.1-min cadence is NOT an arm: `binml.preprocess` pools to one value per 15-min Roman epoch, so after preprocessing the real cadence is indistinguishable from ours (frac ≈ 1 either way). |
| 3 | Detectability-conditioned relabelling of GULLS itself: apply our Δχ² + 0.02 mag floor policy to the noise-free `true_flux_uJy` of GULLS binaries and score recall against detectability labels instead of generator labels | the 1S2L "recall 0.46" counts undetectable planets as misses; this gives the first like-for-like number | — | `validation/gulls/detectability_relabel.py` (extract + reduce), truth cache `~/Desktop/Research/microlensing/gulls_truth_cache` | 🔄 running 2026-09-10 on the first 2,500 dense ids of each binary class + 1,000 1S1L (the obs table is read remotely at 0.2–0.5 s/event for sparse id sets, so the full 56,975 would take a day; the selection is a deterministic prefix and can be extended without redoing anything). Policy applied verbatim: `pipeline.assemble._pspl_refit_dchi2` on the noise-free F146 curve, sigma from GULLS' own `flux_err_uJy`, Δχ² ≥ 500 / 160 and the 0.02 mag floor. Smoke test (40 1S2L): 9 of 40 "planetary" events have NO detectable anomaly by our policy. |
...
| 5 | Noise-model matching: GULLS is 3–4× noisier at the faint end than our photometry; a noise-multiplier augmentation, fine-tune, re-score | not stated | **Premise measured, and it was overstated.** From GULLS' own `flux_err_uJy` at quiescence (3,295 events): GULLS/ours F146 error ratio 0.95 at m_base 17–19, 1.03 at 19–20, 1.20 at 20–21, 1.45 at 21–22, 1.86 at 22–23, 2.23 at 23–24, 2.47 at 24–25 — not 3–4×, and not uniform. A **7× background variance** (`SurveyConfig.bkg_mult = 7`, rms 0.057 in log σ) reproduces the run; a global multiplier does not (best 1.34×, rms 0.23). Physically: crowded-field light, which hurts faint sources only. | `validation/gulls/gulls_noise_vs_ours.py` → `gulls_noise_model.json`; knobs `SurveyConfig.noise_mult/bkg_mult` (defaults bit-identical, `tests/test_noise_mult.py`); regimes `fspl5s_noisy{,_highmag}` | 🔄 running 2026-09-10: `fspl_finetune_local.py --prefix fspl5s_noisy --init ft_g08e12.pt --tag fspl5s_noisy_g08` — the exact fspl5s recipe (same shard seeds, same warm start) with bkg_mult 7, so fspl5s vs fspl5s_noisy isolates the noise model. |
| 6 | Sub-day-t_E single lenses (the FFP-like third of RMDC26 1S1L removed by the t_E ∈ [1, 300] d support cut) as an explicit out-of-support row | not stated | — | `transfer_subday_{shipped,ft_g08e12,fspl_g08,fspl5_g08,fspl5s_g08}.json` (pending) | 🔄 queued 2026-09-10 in the same driver after row 3: 1,500 sub-day 1S1L (t_E < 1 d, amplitude ≥ 0.1 mag, first dense ids), separate curve cache, all five checkpoints. |
