...
| 2 | Schedule-matched augmentation: fine-tune with the exact seven fixed ~6.2 h pauses at 12.1-min cadence in a 70.7-d season (the `arm12` tree from the cadence rerun has the 12-min config) and compare to g08e12's random 1–12 h gaps | not stated | **Schedule-matched is WORSE, and the reason is a known label bug, not the augmentation.** Same pool, same recipe: under the exact schedule held-out AP rand 0.906 / sched 0.805 (completeness at purity 0.81 vs 0.50; NonPSPL recall 0.91 vs 0.80); clean AP 0.951 vs 0.934. Reference rows on the same held-out: shipped 0.211 under the schedule, g08e12 0.887. Mechanism (measured on the pool): `t_anom` is quantised to ten values 7.2 d apart, and two of them (21.6 d, 72.0 d) fall inside the fixed mask, so the caustic-in-gap relabel fires on **20.1% of ALL NonPSPL training events on every presentation** — the sched arm was taught that a fifth of binaries are PSPL. Random gaps dilute the same quantisation error across positions, which is why g08e12/rand never showed it. This is audit finding 9 (`docs/AUDIT_2026-09-09.md`) with a measured cost. | `validation/gulls/schedule_finetune.json`, `validation/schedule_finetune_local.py`, `--gap-schedule`, `--gap-relabel-anomaly` | 🔄 third arm `sched_norelabel` (schedule, relabel off) training 2026-09-10 to attribute cleanly; GULLS scoring of rand/sched in progress | |
...
| 5 | Noise-model matching: GULLS is 3–4× noisier at the faint end than our photometry; a noise-multiplier augmentation, fine-tune, re-score | not stated | **Premise measured, and it was overstated.** From GULLS' own `flux_err_uJy` at quiescence (5,261 events): GULLS/ours F146 error ratio 0.95 at m_base 17–19, 1.04 at 19–20, 1.21 at 20–21, 1.46 at 21–22, 1.89 at 22–23, 2.32 at 23–24, 2.64 at 24–25 — not 3–4×, and not uniform. A **~7× background variance** (`SurveyConfig.bkg_mult`; best fit 7.4, rms 0.062 in log σ) reproduces the run; a global multiplier does not (best 1.41×, rms 0.27). Physically: crowded-field light, which hurts faint sources only. Regime uses 7.0 (fit on the first 3,295 events: 6.9). | `validation/gulls/gulls_noise_vs_ours.py` → `gulls_noise_model.json`; knobs `SurveyConfig.noise_mult/bkg_mult` (defaults bit-identical, `tests/test_noise_mult.py`); regimes `fspl5s_noisy{,_highmag}` | 🔄 fine-tune running 2026-09-10: `fspl_finetune_local.py --prefix fspl5s_noisy --init ft_g08e12.pt --tag fspl5s_noisy_g08` — the exact fspl5s recipe with bkg_mult 7, so fspl5s vs fspl5s_noisy isolates the noise model | |
...
Status 2026-09-10 afternoon: 1, 3, 4, 6, 7 done; 2 and 5 have their training runs in progress. None is needed for the resubmission; each is one paragraph if done. The relabelling result (row 3) changes how the GULLS recall should be quoted: report recall on detectable-anomaly binaries alongside the generator-label number.
