...
experiments, the 2026-09-11 verification's controls and the 2026-09-12 additions are the 22 rows of the ledger in §1½.
...
by a third verification the same day, `docs/VERIFICATION_2026-09-12b.md`).
...
| 23 | Stress suite and training recipe (pre-revision errors found by the third verification's completeness critic, 2026-09-13) | **The 14.9M-event stress suite was scored with stage 5**, the released model's predecessor, and the paper had quoted it as the released model's. `validation/stress_rescore_local.py` regenerates the first shards of each quoted tier with the suite's seeds and original out-of-range class mix (`run_shard --legacy-oor-mix`; 255,527 events) and scores them with both checkpoints. Stage 5 on the subset reproduces the suite (macro-F1 0.927 vs 0.927; median difference 0.012, largest 0.095 on 60 wide-separation binaries). Released model: natural macro-F1 0.919 (its held-out 0.919), anomaly recall/precision 0.962/0.726; low q 0.909/0.325 (precision low only through prevalence, 0.816 at the natural prevalence); wide s 0.417; long P 0.039; faint PSPL 0.590, anomaly precision 0.026 (0.44 at the natural prevalence; false-anomaly rate 2.1% → 5.4%). **The suite's sub-day 'PSPL recall 0.524' was not a single-lens recall**: 38% of the weighted PSPL-labelled events are demoted binaries of natural timescale; the sub-day single lenses are classified PSPL 0.274 (stage 5 0.215) and called anomalies 71% (55%), 37% (21%) above the frozen threshold. Stage 6 had trained on the edges of three sweeps (s 3-8, tE 0.3-10 d, m 23.5-25) and the low-q regime is a stage-4 pool: stated. **The appendix training recipe** described the base run's defaults (batch 256, factor 2 on the anomaly class, 5 epochs); it now gives the six-stage chain from the stage logs. | `validation/stress_rescore_local.json`, Table `tab:stress`, `paper/canonical_numbers.json` infra | Done; Sec. limits, abstract, introduction, appendix, README, model card and evaluation.md rewritten |
