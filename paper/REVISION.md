...
| 2 | Schedule-matched augmentation: fine-tune with the exact seven fixed ~6.2 h pauses at 12.1-min cadence in a 70.7-d season (the `arm12` tree from the cadence rerun has the 12-min config) and compare to g08e12's random 1–12 h gaps | not stated | **Schedule-matched augmentation WINS once a known label bug is removed.** Three arms on ONE pool (the cadence rerun's 15-min natural shards 0–11, 89,919 events), warm-start from the shipped weights, identical recipe (12 epochs, lr 1e-4, truncation 0.5, gap probability 0.8): `rand` = g08e12's random 1–12 h runs; `sched` = the exact RMDC26 mask (seven ~6.2 h pauses + the 1.3 d past the 70.7 d season end); `sched_norelabel` = the same mask with the caustic-in-gap relabel off. Held-out under the exact schedule (30,013 events): AP **rand 0.906 / sched 0.805 / sched_norelabel 0.924**; clean AP 0.951 / 0.934 / 0.952. Reference rows on the same held-out: shipped 0.211, g08e12 0.887 under the schedule. `sched` loses NonPSPL recall (0.80 vs 0.91) because `t_anom` is quantised to ten values 7.2 d apart and two of them (21.6 d, 72.0 d) sit inside the fixed mask, so the relabel fires on **20.1% of ALL NonPSPL training events on every presentation** (measured on the pool) — audit finding 9 with a measured cost; random gaps dilute the same error across positions. With the relabel off, the exact schedule beats the random-gap distribution by 1.8 AP points on the schedule and matches it on clean data. The 12.1-min cadence is not an arm: `binml.preprocess` pools to one value per 15-min epoch, so the real cadence is indistinguishable from ours after preprocessing. GULLS: `rand` replicates g08e12 on a different pool (FA 11.8% vs 11.7%, mean recall 0.455 vs 0.486); `sched` is worse with FLAT false alarms across ρ/\|u₀\| (0.15 even on point sources — the moved boundary); `sched_norelabel` pending. | `validation/gulls/schedule_finetune.json`, `transfer_full_sched_{rand,sched,norelabel}.json`, `validation/schedule_finetune_local.py`, `pipeline/train.py --gap-schedule rmdc26 --gap-relabel-anomaly off`, `tests/test_gap_augmentation.py` | ✅ held-out done 2026-09-10; GULLS scoring of `sched_norelabel` running (disk stall killed the first attempt) |
