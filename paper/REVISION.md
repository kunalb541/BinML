...
| 8 | **Onset fix** (audit finding 9): resolve `t_anom` on the 0.5 d grid the cascade evaluation uses instead of rounding it up to 7.2 d; regenerate the fspl5s pool and rerun the round-3 recipe as `fspl5s_v2_g08` | rows 2/3 showed the old grid mislabels 20% of binaries under a fixed schedule; the fine-tune labels should improve, GULLS numbers should not get worse | — | `SurveyConfig.onset_resolution_days` (0.5; 7.2 reproduces the legacy grid bit-for-bit), `tests/test_onset_resolution.py`, run `fspl_finetune_local.py --prefix fspl5s --tag fspl5s_v2_g08` | 🔄 running 2026-09-11 (pool regenerated with the fine onset; fine-tune in progress; GULLS + calibration follow) |
