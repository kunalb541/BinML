...
| Round 3 `ft_fspl5s_g08.pt` (single-lens rho ≤ 5, binaries unchanged): best at matched budgets, tied by the combined arm (§1½ row 14) — FA 4.8%, recall 0.390 @ 5.2% FA, mean 0.538 | ✅ `fspl_finetune_fspl5s_g08.json`, `transfer_tradeoff_all.json` | ✅ Table gulls |
...
| Figures rebuilt through `build.sh` (weighted prevalence line) | ✅ 2026-09-12: `paper/build.sh` end to end (validate 41 hashed files → figures → both macro generators, fail-closed → LaTeX), 25 pages, no undefined references | — |
...
| 19 | Second verification pass (2026-09-12) | Five verifiers, five adversarial checkers and a completeness critic on the corrected state: 74 findings (6 critical, 22 major, 46 minor), none refuted. Critical: the combined arm's false-alarm localisation (row 14), the physics attribution (row 13), undisclosed per-event weighting (row 18). Also: the macro generator was not fail-closed (tables written before later checks, four silent fallbacks), two values double-rounded, the checkpoint count excluded five scored arms, the "seed spread" was a pool spread, the paper's own single-slice threshold (0.931 for `fspl5s`) was not disclosed, the cascade compared a whole-population three-band number with planetary strata, the Discussion said "recovers most" of the between-season events, and several code paths (cascade/between-season caches keyed by name, a stale-output bug in the fine-tune runner, the onset window end, generation settings lost at caching). Record and dispositions: `docs/VERIFICATION_2026-09-12.md`. | `docs/verification/2026-09-12_findings.json`, `docs/verification/dispositions_2026-09-12.py` | ✅ addressed |
| 20 | Seed replicates of the recommended recipe (2026-09-12) | `fspl5s_seasons_g08` retrained with seeds 20260910 and 20260911 (same pool, recipe and data; code differs only in truth-gated paths that these caches never enter). 1S2L recall at the 5.2% budget per event 0.391 / 0.349 / 0.358 (weighted 0.407 / 0.386 / 0.385); FA at the frozen threshold 6.3 / 4.7 / 7.3%; own held-out AP with the measured pauses 0.893 / 0.876 / 0.894 (clean 0.929 / 0.919 / 0.931); own threshold → RMDC26 FA 2.4 / 2.6 / 3.0%, recall 0.294 / 0.259 / 0.278. Seed 2 against seed 1: −0.042 per event, event-bootstrap interval −0.049 to −0.033 — **the paired intervals exclude zero between seeds of one recipe, so they do not bound recipe differences**. Rule adopted in the paper: no difference between single runs smaller than the seed range in the same weighting (0.042 per event, 0.022 weighted) is interpreted. Survive: extra training (control vs g08e12 +0.066 / +0.038), measured pauses vs random gaps (+0.072 / +0.048), the physics' false-alarm drop in the rho/\|u0\| 1-3 and >3 bins (0.13, 0.22 vs seed range ≤ 0.083). Do not survive: the physics' recall gain (+0.030 / +0.010), the combined arm's lead over `fspl5s` (seeds 2-3 trail it per event by 0.032-0.041 and tie it weighted), its extra frozen-threshold false alarms. Decision: keep seed 1 as `binml-gapaware.pt` (chosen before the replicates; replacing it by the best seed on RMDC26 would be another selection), state that RMDC26 does not separate the recipes and that the released run is the best of three. | `fspl_finetune_fspl5s_seasons_g08_s{2,3}.json`, `transfer_full{,_reduced}_fspl5s_seasons_g08_s{2,3}.json`, `gapped_threshold_fspl5s_seasons_g08_s{2,3}_seasons.json`, `transfer_tradeoff_all.json` (models `_s2`, `_s3`; pairs), `rmdc26_scores.csv.gz` | ✅ in paper |
...
| 22 | Legacy augmentation labels against stored truth (audit findings 8-10) | One natural-prior shard regenerated with `--truth-bins` (7,403 events × 4 presentations) and again with the 0.5-d onset. Truncation: released labels differ from the generator's prefix rule for 11.3% of truncated binaries (6.5% still PSPL after the anomaly is detectable), and from the floors for 16.4% LPV, 8.7% periodic, 5.0% PSPL, 3.1% eruptive presentations. Measured pauses: caustic-in-gap relabel on mislabels 10.5% of binary presentations (off: 1.4%); random gaps 1.2% (0.4%). **Correction the same day:** the first truth rule applied to truncation used full-season-fit residuals, which a prefix refit partly absorbs, and disagreed with the prefix rule on 14.2% (13.7% premature NonPSPL) — worse than legacy; `_apply_truncation` now takes only the floors from the truth and keeps the onset (6.8% with the 7.2-d onset, 0 with the 0.5-d onset). Nothing released was trained with truth relabelling. | `validation/truth_relabel_impact.json`, `tests/test_truth_relabel.py` | ✅ in paper (§training, §limits) |
