...
class FlashAttentionPooling(nn.Module):
    ...
    def forward(self, x: Tensor, lengths: Optional[Tensor] = None) -> Tensor:
        ...
        # NOTE: for COMPACTED inputs `lengths` MUST be passed. If it is None the query
        # attends over the padded tail too, leaking padding/length into the pooled vector
        # (same failure class as unmasked mean pooling). Production always passes lengths;
        # None is valid only for genuinely full (unpadded) sequences.
        attn_mask = None
...
class RomanMicrolensingClassifier(nn.Module):
    ...
    def forward(
        ...
        if self.pooling is not None:
            ...
            # Mean-pooling fallback (use_attention_pooling=False).
            #
            # WARNING -- unmasked mean pooling is a KNOWN SHORTCUT ("cheating"):
            # the sequences fed here are COMPACTED (valid obs in the prefix [0, length))
            # and NORMALIZED, so every padded slot holds the constant value
            # (0 - flux_mean)/flux_std. An unmasked x.mean(dim=1) mixes that constant in
            # with weight (T - length)/T, which leaks the valid-observation COUNT into the
            # pooled representation. If that count correlates with the label (a simulation
            # /masking artifact, not physics), the model learns to classify by COUNTING
            # POINTS rather than reading the light-curve shape -- inflated val accuracy that
            # does not transfer. Attention pooling (the default) is length-invariant and
            # does not have this failure mode.
            #
            # We therefore REQUIRE lengths and only ever do the length-MASKED mean (which
            # divides by the valid count and is length-invariant). Refuse to pool over
            # padding rather than silently cheat.
            if lengths is None:
                raise ValueError(
                    "Mean pooling requires `lengths` to mask padding. An unmasked mean over "
                    "compacted+normalized sequences leaks the valid-observation count and is "
                    "unsafe (see comment). Pass lengths, or use attention pooling "
                    "(use_attention_pooling=True, the default)."
                )
            mask = torch.arange(T, device=device)[None, :] < lengths[:, None]
            mask = mask.float().unsqueeze(-1)  # [B, T, 1]
            # length-invariant masked mean; epsilon guards division by zero
