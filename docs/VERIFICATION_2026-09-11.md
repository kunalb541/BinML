...
| G13 | critical | 'the schedule hides about half of the planets' | Fixed: the gap removes 4-10 points of detectable anomalies relative to in-season; rerun on between-season events only with a multi-start refit. | numbers-draft-14, numbers-ledger-docs-2, rederive-transfer-2, science-relabel-1, referee-logic-6 |
| G14 | major | finite-source gain confounded with 12 extra epochs and a different pool; round-2 penalty understated | Point-source control arm (pspl5s_ctrl, same pool shape and recipe) run; round-2 penalty quantified (FA 8.7% vs 4.8%). | numbers-draft-15, training-experiments-11, referee-logic-15 |
...
| G31 | major | cascade comparison mixes models; untested causal claim; scan details differ from in-house | Fixed: in-house numbers named as the shipped model on 80% stellar binaries, planetary strata quoted; causal wording withdrawn; rescan with >= 10 points per cut and unrounded P. | numbers-ledger-docs-22, referee-logic-17, code-regression-15 |
...
| G44 | critical | RMDC26 described as a pure test set | Disclosed: it guided the diagnosis, the single-lens rho prior and the choice among seven fine-tunes; numbers for the chosen checkpoint are optimistic. | referee-logic-3, training-experiments-10 |
...
| G50 | minor | ESPLMag2 hand-off steps of 4-8 mmag | Fixed: ESPLMag (matches disc integration); legacy flag; round-3 recipe rerun with the smooth function. | code-regression-10 |
...
| G54 | minor | 'exact schedule matches random gaps'; two factors changed at once | Rerun: rand_norelabel and sched_seasons arms; per-season GULLS breakdown. | rederive-transfer-10, training-experiments-13, training-experiments-14 |
