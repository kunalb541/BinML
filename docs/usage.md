...
independent GULLS simulation of the planned survey, on the Hugging Face Hub) to the classifier
under BinML's input contract: pinned dataset revision, one contiguous season found from the epoch
table, baseline measured from the data, remote fetch by event id. Install the extras
(`pip install -e ".[roman]"`) and see `examples/01_classify_roman_event.ipynb`.

```python
import binml
from binml import gulls
seasons, epoch_map, meta = gulls.load_tables()                     # one-time 150 MB download
clf = binml.Classifier(weights="validation/gulls/weights/ft_fspl5s_g08.pt")   # schedule-aware checkpoint
res = gulls.classify_event(clf, 306535, seasons, epoch_map, meta)
res["seasons"][0]["probabilities"]["NonPSPL"] >= 0.949              # its calibrated threshold
```

Use the gap-aware, finite-source checkpoint for this data, not the shipped weights: Roman's
planned schedule pauses F146 seven times per season and the shipped weights were trained on a
continuous grid (README, "known limitations"). `classify_event(..., all_seasons=True)` scores
every high-cadence season and lets you take the maximum, which is how events whose peak falls
between seasons are handled (`paper/REVISION.md` §1½ row 9). The population-level numbers are
regenerated by the scripts under `validation/gulls/`.
