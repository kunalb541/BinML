...
clf = binml.Classifier(weights="validation/gulls/weights/ft_fspl5s_g08.pt")   # gap-aware, finite-source
cal = json.load(open("validation/gulls/gapped_threshold_fspl5s_g08_seasons.json"))
threshold = cal["arms"]["rmdc26_gapped"]["pool"]["full_pool"]["threshold"]    # 0.957
...
Use the gap-aware, finite-source checkpoint for this data, not the shipped weights: RMDC26 pauses
