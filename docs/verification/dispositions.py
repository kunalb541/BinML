...
ISSUES = {
 ...
 "G13": ("'the schedule hides about half of the planets'", "Fixed: the gap removes 4-10 points of detectable anomalies relative to in-season; rerun on between-season events only with a multi-start refit."),
 "G14": ("finite-source gain confounded with 12 extra epochs and a different pool; round-2 penalty understated", "Point-source control arm (pspl5s_ctrl, same pool shape and recipe) run; round-2 penalty quantified (FA 8.7% vs 4.8%)."),
 ...
 "G31": ("cascade comparison mixes models; untested causal claim; scan details differ from in-house", "Fixed: in-house numbers named as the shipped model on 80% stellar binaries, planetary strata quoted; causal wording withdrawn; rescan with >= 10 points per cut and unrounded P."),
 ...
 "G50": ("ESPLMag2 hand-off steps of 4-8 mmag", "Fixed: ESPLMag (matches disc integration); legacy flag; round-3 recipe rerun with the smooth function."),
 ...
 "G54": ("'exact schedule matches random gaps'; two factors changed at once", "Rerun: rand_norelabel and sched_seasons arms; per-season GULLS breakdown."),
